    ...
    update(time, context) {
        ...
        if (highX - lowX > context.canvas.width - SCROLL_BOUNDRY * 2) {
            ...
            for (const fighter of this.fighters) {
                if(fighter.position.x < this.position.x + SCROLL_BOUNDRY && fighter.velocity.x * fighter.direction < 0 
                    || fighter.position.x > this.position.x + context.canvas.width - SCROLL_BOUNDRY && fighter.velocity.x * fighter.direction > 0) 
                    {
